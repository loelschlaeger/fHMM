...
The [fHMM](https://loelschlaeger.de/fHMM/) R package allows for the
detection and characterization of financial market regimes in time
series data by applying hidden Markov Models (HMMs). The
[vignettes](https://loelschlaeger.de/fHMM/articles/) outline the package
functionality and the model formulation.
...
The [`summary()`](https://rdrr.io/r/base/summary.html) method gives an
overview:
...
![](reference/figures/README-plot_model_fit-1.png)![](reference/figures/README-plot_model_fit-2.png)
...
![](reference/figures/README-plot_model_residuals-1.png)

## Simulating HMM data

The [fHMM](https://loelschlaeger.de/fHMM/) package supports data
simulation from an HMM and access to the model likelihood function for
model fitting and the Viterbi algorithm for state decoding.
...
2.  Define the model parameters via the
    [`fHMM_parameters()`](https://loelschlaeger.de/fHMM/reference/fHMM_parameters.md)
    function (unspecified parameters would be set at random).
...
3.  Simulate data points from this model via the
...
![](reference/figures/README-simulate-hmm-data-1.png)

4.  The log-likelihood function
    [`ll_hmm()`](https://loelschlaeger.de/fHMM/reference/ll_hmm.md) is
    evaluated at the identified and unconstrained parameter values, they
...
5.  For maximum likelihood estimation of the model parameters, we can
    ...
    [`ll_hmm()`](https://loelschlaeger.de/fHMM/reference/ll_hmm.md) over
    `parUncon` (or rather minimize the negative log-likelihood).
...
6.  To interpret the estimate, it needs to be back-transformed to the
    constrained parameter space via the
    [`parUncon2par()`](https://loelschlaeger.de/fHMM/reference/parameter_transformations.md)
    function. The state labeling is not identified.
