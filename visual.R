...
create_visuals = function(data,fit,decoding,controls,labels=NULL){
  ...
  ### save labels
  if(!controls[["sim"]] & !is.null(labels)) check_saving(labels,controls)
  ...
  colors = matrix(0,nrow=controls[["states"]][1],ncol=controls[["states"]][2]+1)
  colors[,1] = col_alpha(base_col(controls[["states"]][1]))
  for(s in seq_len(controls[["states"]][1])) colors[s,-1] = col_alpha(var_col(colors[s,1],controls[["states"]][2]))
  
  ### define legend layout
  legend_layout = list(cex=1.25,x="topleft",bg="white")
  
  ### create visualization of state dependent distributions
  plot_sdds(data,decodings,controls,colors,legend_layout)
  
  ### create visualization of decoded time series
  #plot_ts(data,decodings,controls)
  
  ### compute, save and visualize pseudo-residuals
  #plot_prs(data,decodings,controls)
}

### create visualization of state dependent distributions
plot_sdds = function(data,decodings,controls,colors,legend_layout){
  
  ### extract parameters
  states = controls[["states"]]
  T = dim(t(data[["logReturns"]]))[1]
  ...
  if(controls[["overwrite"]]==FALSE & file.exists(filename)){
    ...
    if(controls[["model"]]=="HMM"){
      ...
      for(s in seq_len(states[1])){
        sdd_density_values[[s]] = sdd_density(fit[["thetaList"]][["mus"]],fit[["thetaList"]][["sigmas"]],fit[["thetaList"]][["dfs"]],s,x)
      }
      ymin = 0
      ymax = max(unlist(sdd_density_values))
      xmin = min(rep(x,states[1])[unlist(sdd_density_values)>trunc_distr])
      xmax = max(rep(x,states[1])[unlist(sdd_density_values)>trunc_distr])
      ...
      pdf(filename, width=8, height=8)
        ...
        axis(1,c(xmin,0,xmax),labels=sprintf("%.2f",c(xmin,0,xmax)))
        axis(2,seq(ymin,ymax,by=20),las=1)
        title(main = "State-dependent distributions",
              xlab = "Log-return",
              ylab = "Density")
        do.call(legend,c(list(legend=paste("State",seq_len(states[1])),col=colors[,1],lwd=lwd),legend_layout))
        for(s in seq_len(states[1])){
          lines(x[sdd_density_values[[s]]>trunc_distr],sdd_density_values[[s]][sdd_density_values[[s]]>trunc_distr],col=colors[s,1],lwd=lwd)
        }
      invisible(dev.off())
    }
  ...
  writeLines("Visualization of state-dependent distributions successful.")
}
...
plot_ts = function(){
  ...
  writeLines("Visualization of decoded time series successful.")
}
...
plot_prs = function(){
  ...
  writeLines("Visualization of pseudo-residuals successful.")
}
