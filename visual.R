...
visual = function(data,fit,decoding,controls,labels=NULL){
  ...
  ### decoded time series
  pdf(paste0("models/",controls[["model_name"]],"/ts.pdf"), width=19, height=9)
  ...
  ymin = floor(min(data$closes)/1000)*1000 - (ceiling(max(data$closes)/1000)*1000-floor(min(data$closes)/1000)*1000)/1.5
  ...
  par(las=3)
  mtext("closing price", side=4, line=4, at=mean(data$closes), cex=1.5)
  par(las=1)
  ...
  axis(4, seq(floor(min(data$closes)/1000)*1000,ceiling(max(data$closes)/1000)*1000,length.out=5))
  if(controls[["model"]]=="HMM"){
    for(s in seq_len(states[1])){
  ...
  if(controls[["model"]]=="HHMM"){
    ...
    eg = expand.grid(seq_len(states[1]),seq_len(states[2]))
    do.call(legend,c(list(legend=paste0("coarse-scale state ",eg[,1],", fine-scale state ",eg[,2]),col=colors[,-1],pch=19),legend_layout))
  }
  if(!controls[["sim"]]){
    for(l in seq_len(length(labels[["dates"]]))){
      text(as.Date(labels[["dates"]][l]),max(data$closes),labels[["names"]][l],pos=4,cex=1.5)
  ...
  if(controls[["model"]]=="HMM"){
    ...
    plot(data$dates,data$observations,type="h",col="grey",xlab="",ylab="",xaxt="n",yaxt="n",xlim=c(xmin,xmax),ylim=c(ymin,ymax+0.2))
  }
  if(controls[["model"]]=="HHMM"){
    ymin = min(data$observations[,-1]); ymax = max(data$observations[,-1])
    plot(data$dates,data$observations[,-1],type="h",col="grey",xlab="",ylab="",xaxt="n",yaxt="n",xlim=c(xmin,xmax),ylim=c(ymin,ymax+0.2))
  }
  par(las=3)
  mtext("log-return", side=2, line=4, at=0, cex=1.5)
  par(las=1)
  axis(2, round(seq(ymin,ymax,by=0.04),2))
  if(controls[["model"]]=="HMM"){
    for(s in seq_len(states[1])){
  ...
  if(controls[["model"]]=="HHMM"){
    for(cs in seq_len(states[1])){
      for(fs in seq_len(states[2])){
        points(data$dates[decoding_cs==cs&decoding_fs==fs],as.vector(t(data$observations[,-1]))[decoding_cs==cs&decoding_fs==fs],col=colors[cs,fs+1],pch=20)
      }
    }
  }
  invisible(dev.off())
