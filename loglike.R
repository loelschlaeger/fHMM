...
logL_hmm = function(x,thetaCon,N){
      T = length(x)
...
# INPUT:  unconstrained parameter vector, observations, model controls
...
logL_hhmm = function(thetaUncon,observations,controls){
      ...
      est_df = controls[["est_df"]]
			...
			for (i in 1:M){
			  log_likelihoods[i,] = apply(x[,-1], 1, logL_hmm, thetaCon_step[[i]], N)
			}
