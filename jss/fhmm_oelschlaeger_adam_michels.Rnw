...
%% Software overview
R-packages: general-purpose: \pkg{hmm} \citep{him10}, \pkg{depmixS4} \citep{vis10}, \pkg{msm} \citep{jac11}, ... Special-puropose: animal movement modeling: \pkg{moveHMM} \citep{mic16}, \pkg{momentuHMM} \citep{mcc18}. Hidden semi-Markov models: \pkg{hsmm} \citep{bul10}, \pkg{mhsmm} \citep{oco11}. Count data: \pkg{hmm.discnp} \citep{tur14}, \pkg{countHMM} \citep{ada19b}. Hidden Markov models have also been implemented in Python, most notably in the \pkg{hmmlearn} package \citep{leb22}.
...
These lines first set the number \code{states = 3} of hidden states. Any number greater or equal 2 is possible. Next, \code{sdds = "t"} specifies state-dependent t-distributions, which is the common choice for modeling log-returns \citep{pla08}. Alternatively, \code{sdds = "gamma"} specifies Gamma distributions, which is useful to model trading volumes as in \cite{ada20}. The \code{data} entry sets the path to the file (\code{file = "dax.csv"}), the file's column that contains the dates (\code{date_column = "Date"}) and the data (\code{date_column = "Close"}). The command \code{logreturns = TRUE} transforms the data to log-returns. Passing the list to the \fct{set\_controls} function returns an object of class \class{fHMM\_controls}, which contains the specifications and default settings.
...
\paragraph{Example 3: Hierarchy} To specify an hierarchical HMM, add \code{hierarchy = TRUE} to the controls. The following lines show an example specification for the DAX and the Volkswagen stock:
...
The following line applies the \fct{prepare\_data} function to control specification for the 3-state HMM DAX model from the previous section. The output object can be fed into the \fct{summary} method for a quick data overview:
...
Additionally, the data can be visualized via the \fct{plot} method. To facilate interpretation, historical events can be highlighted by specifying a named list \code{events} with elements \code{dates} (a vector of dates) and \code{labels} (a vector of labels for the events) and passing it to the plot method, for example:
