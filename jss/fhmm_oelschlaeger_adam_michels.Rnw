...
%% Intro to HMMs
In recent years, hidden Markov models (HMMs) have emerged as a popular tool for modeling time series that are subject to state-switching over time \cite{zuc16}. In finance, these hidden states can often be linked to market regimes such as bearish and bullish markets or recessions and periods of economics growths, to name but a few examples.

%% Literature review
Over the last decades, they have been applied to model business cycles, to derive stylized facts of stock returns \cite{bul06, nys15a}, and to model energy prices conditional on market states \cite{lan18, ada22}, to name but a few examples.
