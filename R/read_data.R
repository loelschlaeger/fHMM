...
read_data = function(controls){
  ...
  if(controls[["hierarchy"]]){
    ...
    for(t in seq_len(T)){
      data[t,-1] = c(data_raw[[2]][[col_name]][(sum(T_star[seq_len(t-1)])+1):sum(T_star[seq_len(t)])],
                     rep(NA,max(T_star)-T_star[t]))
      time_series[t,-1] = c(data_raw[[2]][[data_column[2]]][(sum(T_star[seq_len(t-1)])+1):sum(T_star[seq_len(t)])],
                            rep(NA,max(T_star)-T_star[t]))
    }
  ...
  if(!all(is.na(date_column))){
    ...
    if(controls[["hierarchy"]]){
      ...
      for(t in seq_len(T))
        dates[t,-1] = c(data_raw[[2]][[date_column[2]]][(sum(T_star[seq_len(t-1)])+1):sum(T_star[seq_len(t)])],
                       rep(NA,max(T_star)-T_star[t]))
      dates[,1] = dates[,2]
    } else {
