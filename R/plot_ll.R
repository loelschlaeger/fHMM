...
plot_ll = function(lls){
  if(length(lls) <= 5){
    plot(lls, xaxt = "n", yaxt = "n", xlab = "Estimation run", ylab = "", 
    ...
    plot(lls, yaxt = "n", xlab = "Estimation run", ylab = "",
         main = "Log-likelihoods", pch = 16, 
         ylim = c(floor(min(lls,na.rm=TRUE)), ceiling(max(lls,na.rm=TRUE))))
  }
  points(x = which.max(lls), y = lls[which.max(lls)], pch = 16, cex = 1.25, 
         col = "red")
  axis(2, las = 1, at = unique(round(lls[!is.na(lls)])),
