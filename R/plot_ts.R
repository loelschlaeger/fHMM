...
plot_ts <- function (data, decoding, colors, events) {
  ...
  if (!controls[["simulated"]]) {
    ...
    xmax = as.Date(paste0(as.numeric(format(as.Date(tail(data[["dates"]], n = 1)), "%Y")) + 1, "-01-01"))
    if (!controls[["hierarchy"]]) {
    ...
    ymin = -ymax
    plot(as.Date(data[["dates"]]), ydata, type = "l", 
    ...
    if (!controls[["hierarchy"]]) {
      for (s in seq_len(controls[["states"]][1])) {
        points(as.Date(data[["dates"]][decoding == s]), ydata[decoding == s], col = colors[s], pch = 20)
      }
    }
    if (controls[["hierarchy"]]) {
      for (cs in seq_len(controls[["states"]][1])) {
        for (fs in seq_len(controls[["states"]][2])) {
          points(data[["dates"]][decoding_cs == cs & decoding_fs == fs], 
                 ydata[decoding_cs == cs & decoding_fs == fs], col = colors[["hhmm_fs"]][[cs]][fs], 
                 pch = 20)
        }
      }
    }
    par(new = TRUE, las = 1)
    x_values = as.Date(data[["dates"]])
    ymax_factor = 3
  }
  if (controls[["simulated"]]) {
    xmin = 1
  ...
  if (!controls[["simulated"]] & !is.null(events)) {
    events[["labels"]] = events[["labels"]][events[["dates"]] > 
  ...
  if (controls[["hierarchy"]] & !is.null(decoding)) {
    ...
    legend(legend = c(paste("Coarse-scale state", seq_len(controls[["states"]][1])), 
           ...
           col = c(colors[["hhmm_cs"]], as.vector(unlist(colors[["hhmm_fs"]]))), 
           ...
           cex = 1.25, bg = rgb(1, 1, 1, 0.5), x = "topleft")
  }
  if (controls[["hierarchy"]]) {
    par(new = TRUE)
    ymin = min(cs_data)
    ...
    for (cs in seq_len(controls[["states"]][1])) {
      points(x_values_cs[decoding[, 1] == cs], cs_data[decoding[,1] == cs], 
             col = colors[["hhmm_cs"]][[cs]], pch = 1, cex = 3, lwd = 2)
    }
    if (ymin < 0 & 0 < ymax) {
    ...
    if (controls[["simulated"]]) {
      mtext("Simulated coarse-scale data", side = 4, line = 3.5, 
            at = mean(c(ymin, ymax)), cex = 1.25, las = 3)
    }
    if (!controls[["simulated"]]) {
      mtext("Coarse-scale data", side = 4, line = 3.5, at = mean(c(ymin, ymax)), 
            cex = 1.25, las = 3)
    }
  }
}