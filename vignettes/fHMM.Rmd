...
Welcome to fHMM, a package for modeling financial time series with hidden Markov models (HMMs). This introductory vignette [motivates the approach](#motivation), gives an [overview](#package-and-vignettes-overview) of the package functionality and the vignettes, informs about [debugging](#debugging) options, and [places the approach in the existing literature](#placement-in-the-literature).
...
- The vignette [*Model estimation*](http://loelschlaeger.de/fHMM/articles/v04_model_estimation.html) defines the likelihood function and explain the task of its numerical maximization. Subsequently, the `fit_model()` function is introduced that receives an `fHMM_data`, fits an HMM (specified by `fHMM_controls`) and returns an `fHMM_model` object, for which the package provides `plot`, `print`, and `summary` methods. 
