test_that("input checks for data download work", {
  expect_error(download_data(symbol = c("two", "symbols")))
  expect_error(download_data(symbol = "^GDAXI", file = c("two", "filess")))
  expect_error(download_data(symbol = "^GDAXI", file = paste0(tempfile(), ".csv"), verbose = "not_TRUE_or_FALSE"))
  expect_message(download_data(symbol = "^GDAXI", file = paste0(tempfile(), ".csv"), verbose = TRUE))
})
