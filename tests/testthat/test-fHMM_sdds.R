test_that("splitting pars parameters from a ssd works", {
  ...
  expect_equal(unclass(fHMM_sdds(sdds = "t(mu = 1|2, sigma = 3, df = 4)")),
  ...
  expect_equal(unclass(fHMM_sdds(sdds = c("t(mu = 1|2, sigma = 3|4, df = 5|6)","gamma(sigma = 1|2|3)"))),
               list(list(name = "t", pars = list("mu" = 1:2, "sigma" = 3:4, "df" = 5:6)),
