test_that("HMM simulation works", {
  expect_error(
    simulate_hmm(true_parameters = NULL),
    "'true_parameters' is not of class 'fHMM_parameters'."
  )
  expect_named(
    simulate_hmm(states = 2, sdds = "normal", horizon = 20),
    c("observations", "states", "parameter")
  )
})

test_that("HHMM simulation works", {
  expect_named(
    simulate_hmm(
      hierarchy = TRUE, states = c(2, 3), sdds = c("gamma", "poisson"),
      horizon = c(10, NA), period = "w"
    ),
    c("observations", "states", "parameter", "observations_fs", "states_fs")
  )
})

test_that("computation of T_star works", {
  expect_error(
    compute_T_star(hierarchy = FALSE),
    "only relevant for hierarchical HMMs"
  )
  expect_equal(
